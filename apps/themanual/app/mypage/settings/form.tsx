...
const SETTINGS_KEY = 'themanual:user-settings'
...
const FIELDS = ['창업 준비', '세무·회계', '노무·인사', '마케팅', '운영 관리', '메뉴 개발', '위생·안전', '기타']
    ...
    <form onSubmit={submit} className="mx-auto max-w-3xl space-y-6">
      ...
      <Card className="border-gray-200 shadow-sm">
        <CardContent className="space-y-3 p-6">
          ...
          <Toggle
            label="새 강좌 알림"
            sub="관심 분야 신규 강좌 등록 시 알림"
            checked={state.notifyCourse}
            onChange={(v) => update('notifyCourse', v)}
          />
          <Toggle
            label="수강 현황"
            sub="수강 기한 만료 전 리마인드 알림"
            checked={state.notifyProgress}
            onChange={(v) => update('notifyProgress', v)}
          />
          <Toggle
            label="자격증 안내"
            sub="관련 자격증·시험 일정 안내"
            checked={state.notifyCert}
            onChange={(v) => update('notifyCert', v)}
          />
        </CardContent>
      </Card>
