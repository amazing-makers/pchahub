...
const SETTINGS_KEY = 'gongganhansu:user-settings'
...
interface FormState {
  ...
  notifyInsight: boolean
  marketingConsent: boolean
}

const REGIONS = ['서울', '경기', '인천', '부산', '대구', '대전', '광주', '울산', '세종', '강원', '충북', '충남', '전북', '전남', '경북', '경남', '제주']
const BUDGETS = ['~ 1,000만원', '1,000 ~ 3,000만원', '3,000 ~ 5,000만원', '5,000만 ~ 1억', '1억 이상']
...
export function SettingsForm({ defaultName, email, role }: SettingsFormProps) {
  ...
  const roleLabel =
    role === 'contractor' ? '시공·인테리어업체'
    : role === 'landlord' ? '건물주·임대인'
    : '임차인'
    ...
    <form onSubmit={submit} className="mx-auto max-w-3xl space-y-6">
      ...
      <Card className="border-gray-200 shadow-sm">
        <CardContent className="space-y-3 p-6">
          ...
          <Toggle
            label="신규 포트폴리오"
            sub="관심 지역 신규 시공 포트폴리오 알림"
            checked={state.notifyPortfolio}
            onChange={(v) => update('notifyPortfolio', v)}
          />
          <Toggle
            label="상권 인사이트"
            sub="공간한수 인사이트 리포트 발송"
            checked={state.notifyInsight}
            onChange={(v) => update('notifyInsight', v)}
          />
        </CardContent>
      </Card>
