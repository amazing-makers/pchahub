...
export default function PchabridgeContactForm() {
    ...
    try {
      const res = await fetch('/api/inquiries', {
        ...
        body: JSON.stringify({
          ...
          type: 'investment',
          metadata: form.investAmount ? { investAmount: form.investAmount } : undefined,
        }),
      })
      if (!res.ok) throw new Error('서버 오류')
      setStatus('success')
      setForm({ name: '', email: '', phone: '', investAmount: '', subject: '', message: '' })
    } catch {
      setStatus('error')
    }
  }
  ...
  if (status === 'success') {
    return (
      <div className="rounded-xl border border-green-200 bg-green-50 p-8 text-center">
        <p className="text-lg font-semibold text-green-800">투자 문의가 접수되었습니다.</p>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        <label className="mb-1.5 block text-sm font-medium text-gray-700">투자 규모</label>
        <input
          name="investAmount"
      ...
      <div>
        ...
        <textarea
          ...
          required
          rows={5}
          placeholder="투자 관련 문의 내용을 자세히 적어주세요."
          className="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm outline-none focus:border-gray-900 focus:ring-1 focus:ring-gray-900"
        />
      </div>
      {status === 'error' && (
