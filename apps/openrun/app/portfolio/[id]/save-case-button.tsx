...
export function SaveCaseButton({ caseId }: { caseId: string }) {
  ...
  function toggle() {
    try {
      const raw = window.localStorage.getItem(KEY)
      const ids: string[] = raw ? JSON.parse(raw) : []
      const next = saved ? ids.filter((id) => id !== caseId) : [...ids, caseId]
