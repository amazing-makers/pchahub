...
const SETTINGS_KEY = 'openrun:user-settings'
    ...
    <form onSubmit={submit} className="mx-auto max-w-3xl space-y-6">
      ...
      <Card className="border-gray-200 shadow-sm">
        <CardContent className="space-y-3 p-6">
          ...
          <Toggle
            label="캠페인 진행 현황"
            sub="의뢰한 캠페인의 상태 변경 시 알림"
            checked={state.notifyCampaign}
            onChange={(v) => update('notifyCampaign', v)}
          />
          <Toggle
            label="캠페인 리포트"
            sub="완료된 캠페인 결과 리포트 발송"
            checked={state.notifyReport}
            onChange={(v) => update('notifyReport', v)}
          />
          <Toggle
            label="신규 서비스 안내"
            sub="오픈런 신규 마케팅 패키지 안내"
            checked={state.notifyNew}
            onChange={(v) => update('notifyNew', v)}
          />
        </CardContent>
      </Card>
