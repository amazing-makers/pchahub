...
const SETTINGS_KEY = 'themyungdang:user-settings'
...
const BUDGETS = ['~ 1,000만원', '1,000 ~ 3,000만원', '3,000 ~ 5,000만원', '5,000만 ~ 1억', '1억 이상', '무관']
...
export function SettingsForm({ defaultName, email, role }: SettingsFormProps) {
  ...
  const roleLabel =
    role === 'seller' ? '매물 올리는 분'
    : role === 'agent' ? '부동산·공인중개사'
    : '매물 찾는 분'
    ...
    <form onSubmit={submit} className="mx-auto max-w-3xl space-y-6">
      ...
      <Card className="border-gray-200 shadow-sm">
        <CardContent className="space-y-3 p-6">
          ...
          <Toggle
            label="새 매물 알림"
            sub="관심 지역 신규 매물 등록 시 알림"
            checked={state.notifyListing}
            onChange={(v) => update('notifyListing', v)}
          />
          <Toggle
            label="가격 변동 알림"
            sub="관심 저장 매물 가격 변동 시 알림"
            checked={state.notifyPrice}
            onChange={(v) => update('notifyPrice', v)}
          />
          <Toggle
            label="상권 리포트"
            sub="매주 주요 상권 분석 리포트 발송"
            checked={state.notifyArea}
            onChange={(v) => update('notifyArea', v)}
          />
        </CardContent>
      </Card>
