...
export default function ThemyungdangContactForm() {
    ...
    try {
      const res = await fetch('/api/inquiries', {
        ...
        body: JSON.stringify({
          ...
          type: 'listing',
          metadata: form.address ? { address: form.address } : undefined,
        }),
      })
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        <label className="mb-1.5 block text-sm font-medium text-gray-700">매물 주소</label>
        <input
          name="address"
          value={form.address}
          onChange={handleChange}
          placeholder="관심 매물 주소 또는 지역"
          className="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm outline-none focus:border-gray-900 focus:ring-1 focus:ring-gray-900"
        />
      </div>
      <div>
