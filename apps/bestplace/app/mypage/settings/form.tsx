...
const SETTINGS_KEY = 'bestplace:user-settings'
    ...
    <form onSubmit={submit} className="mx-auto max-w-3xl space-y-6">
      ...
      <Card className="border-gray-200 shadow-sm">
        <CardContent className="space-y-3 p-6">
          ...
          <Toggle
            label="어워드 결과 발표"
            sub="어워드 투표 결과 및 수상 발표 알림"
            checked={state.notifyAward}
            onChange={(v) => update('notifyAward', v)}
          />
          <Toggle
            label="신규 매장 등록"
            sub="관심 지역 신규 등록 매장 알림"
            checked={state.notifyNewStore}
            onChange={(v) => update('notifyNewStore', v)}
          />
          <Toggle
            label="실시간 랭킹 변동"
            sub="매주 랭킹 변동 알림"
            checked={state.notifyRanking}
            onChange={(v) => update('notifyRanking', v)}
          />
        </CardContent>
      </Card>
