...
import { ArrowRight, Building2, Hammer, Search, User } from 'lucide-react'
...
type Role = 'user' | 'franchisee' | 'hq' | 'contractor'

const ROLES: Array<{ key: Role; label: string; helper: string; icon: typeof User }> = [
  { key: 'user', label: '예비 창업자', helper: '창업 검토 중. 정보·매칭 받기', icon: Search },
  { key: 'franchisee', label: '현직 가맹점주', helper: '운영 중. 정보·후기 공유', icon: User },
  { key: 'hq', label: '본사 / 가맹사업', helper: '브랜드 등록·점주 모집', icon: Building2 },
  { key: 'contractor', label: '시공·협력사', helper: '인테리어·물품 공급', icon: Hammer },
]
...
export function SignupForm({ enabled }: SignupFormProps) {
  ...
  const proceedDev = () => {
    // For dev mode, role is captured during the dev sign-in form on /auth/signin.
    // Bounce them through with prefilled role.
