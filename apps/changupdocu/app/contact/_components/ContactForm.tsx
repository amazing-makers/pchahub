...
export default function ChangupdocuContactForm() {
  ...
  if (status === 'success') {
    return (
      <div className="rounded-xl border border-green-200 bg-green-50 p-8 text-center">
        <p className="text-lg font-semibold text-green-800">제보가 접수되었습니다.</p>
        <p className="mt-1 text-sm text-green-600">검토 후 연락드리겠습니다.</p>
        <button
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <input
          ...
          onChange={handleChange}
          required
          placeholder="제보하실 스토리 제목"
          className="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm outline-none focus:border-gray-900 focus:ring-1 focus:ring-gray-900"
        />
      </div>
      <div>
        <label className="mb-1.5 block text-sm font-medium text-gray-700">
          스토리 내용 <span className="text-red-500">*</span>
        </label>
        <textarea
          name="message"
          value={form.message}
