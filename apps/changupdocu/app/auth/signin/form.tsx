    ...
    <Card className="w-full max-w-md border-gray-200 shadow-sm">
      <CardContent className="p-8">
        <div className="text-center">
          <h1 className="text-h3 font-bold text-gray-900">창업다큐 로그인</h1>
          <p className="mt-1 text-sm text-gray-500">amakers 한 계정으로 9개 사이트 함께.</p>
        </div>

        {error && (
          <div className="mt-5 flex items-start gap-2 rounded-lg bg-rose-50 p-3 text-sm text-rose-700">
            <AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />
            <span>로그인 중 문제. <span className="font-mono text-xs">({error})</span></span>
          </div>
        )}
        ...
        {!hasRealOAuth && (
          <div className="mt-6 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
            OAuth 키가 아직 설정되지 않았습니다. 아래 개발용 로그인 사용.
          </div>
        )}
