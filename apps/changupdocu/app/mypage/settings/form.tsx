...
const SETTINGS_KEY = 'changupdocu:user-settings'
    ...
    <form onSubmit={submit} className="mx-auto max-w-3xl space-y-6">
      ...
      <Card className="border-gray-200 shadow-sm">
        <CardContent className="space-y-3 p-6">
          ...
          <Toggle
            label="새 에피소드"
            sub="새 다큐멘터리 에피소드가 업로드되면 알림"
            checked={state.notifyEpisode}
            onChange={(v) => update('notifyEpisode', v)}
          />
          <Toggle
            label="매거진 추천"
            sub="이번 주 추천 매거진 글 발송"
            checked={state.notifyMagazine}
            onChange={(v) => update('notifyMagazine', v)}
          />
          <Toggle
            label="기고 안내"
            sub="기고 공모·전문가 인터뷰 섭외 알림"
            checked={state.notifyContrib}
            onChange={(v) => update('notifyContrib', v)}
          />
        </CardContent>
      </Card>
